...
<section class="content-header">
  <h1> {{ __('messages.purchase_product_amendment') }}<small></small> </h1>
  <ol class="breadcrumb">
    ...
    <div class="col-md-12">
      <div class="box box-primary">
        <div class="box-header with-border">
          <h3 class="box-title"> <i class="fa fa-list-alt"></i> {{ __('messages.purchase_product_amendment') }}</h3>
        </div>
        <!-- /.box-header -->
        <div class="box-body">
          <div class="row">
                  ...
                  <tbody>
                    <tr>
                      <td style="text-align: center;">
                        <form method="post" action="{{ route('product-edit.filter') }}">
                ...
                <table class="table table-bordered table-striped table-responsive table-hover myTable"> 
                  ...
                  <tbody> 
                    ...
                    <tr> 
                      ...
                      <td><a href="{{$baseUrl.'/'.config('app.purchase').'/purchase-report/'.$data->tok}}" target="_blank">{{$data->tok}}</a></td>
                      <td>{{$data->productpurchase_supplier_object->name}}</td>
                      <td>{{$data->discount}}</td>
                      <td>{{Session::get('currency')}} {{$data->sub_total}}</td>
                      <td> 
                        @if(Auth::user()->type==1)
                        <div class="form-inline">
                          <div class = "input-group">
                            <a href="{{$baseUrl.'/'.config('app.amendment').'/purchase-product-edit/'.$data->tok.'/edit'}}" class="btn btn-success btn-xs">Edit</a>
                          </div>
                          <div class = "input-group">
                            {{Form::open(array('route'=>['purchase-product-edit.destroy',$data->tok],'method'=>'DELETE'))}}
