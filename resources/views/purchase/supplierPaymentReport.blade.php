...
@section('title', 'Exporter Payment Report')
@section('content')
...
<section class="content-header">
  <h1> {{ __('messages.exporter_payment_report') }} <small></small> </h1>
  <ol class="breadcrumb">
    ...
    <div class="col-md-12">
      <div class="box box-primary">
        <div class="box-header with-border">
          <h3 class="box-title"> <i class="fa fa-list-alt"></i> {{ __('messages.exporter_payment_report') }}</h3>
                  ...
                  <tbody>
                    <tr>
                      <td style="text-align: center;">
                        <form method="post" action="{{ route('suplier-pay-report.filter') }}">
                ...
                <table style="width: 100%; font-size: 14px;" cellspacing="0" cellpadding="0"> 
                  <thead> 
                    <tr style="background: #ccc;"> 
                      ...
                      <!--<th style="font-weight: bold; border: 1px solid #ddd; padding: 3px 3px">{{ __('messages.company_name') }}</th>-->
                  ...
                  <tbody> 
                    ...
                    <tr> 
                      ...
                      <td style="border: 1px solid #ddd; padding: 3px 3px">{{$data->date}}</td>
                      <!--td><a href="{{ url('supplier-ledger/'.$data->supplier_id) }}" target="_blank">{{$data->ledger_supplier_object->name}}</a></td-->
                      <td style="border: 1px solid #ddd; padding: 3px 3px"><a href="{{$baseUrl.'/'.config('app.supplier').'/exporter-ledger/'.$data->supplier_id}}" target="_blank">{{$data->ledger_supplier_object->name}}</a></td>
                      <!--<td style="border: 1px solid #ddd; padding: 3px 3px">{{$data->ledger_supplier_object->company}}</td>-->
                      <td style="border: 1px solid #ddd; padding: 3px 3px">{{$data->ledger_supplier_object->phone}}</td>
                      <td style="border: 1px solid #ddd; padding: 3px 3px">{{$data->ledger_supplier_object->address}}</td>
