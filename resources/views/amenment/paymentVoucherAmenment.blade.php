...
@section('title', 'Other Payment Voucher')
@section('content')
<!-- Content Header (Page header) -->
<section class="content-header">
  <h1> {{ __('messages.payment_voucher_report_amendment') }} <small></small> </h1>
  <ol class="breadcrumb">
    ...
    <div class="col-md-12">
      <div class="box box-primary">
        <div class="box-header with-border">
          <h3 class="box-title"> <i class="fa fa-list-alt"></i> {{ __('messages.payment_voucher_report_amendment') }}</h3>
        </div>
        <!-- /.box-header -->
        <div class="box-body">
          <div class="row">
                ...
                <table class="table table-bordered table-striped table-responsive table-hover"> 
                  ...
                  <tbody> 
                    ...
                    <tr> 
                      ...
                      <td>{{dateFormateForView($data->payment_date)}}</td>
                      <td>{{$data->otherpayment_bank_object->bank_name}}</td>
                      <td>{{$data->otherpayment_type_object->name}}</td>
                      <td>{{$data->otherpayment_subtype_object->name}}</td>
                      <td>{{$data->issue_by}}</td>
                      <td>{{$data->otherpayment_user_object->name}}</td>
                      <td>{{$data->amount}}</td>
                      <td> 
                        @if(Auth::user()->type==1)
                        <div class="form-inline">
                          ...
                          <div class="input-group">
                            {{Form::open(array('route'=>['other-payment-amenment.destroy',$data->tok],'method'=>'DELETE'))}}
                              ...
                              <div class="modal-dialog modal-sm">
                                ...
                                <div class="modal-content">
                                  <div class="modal-header">
                                    ...
                                    <h4 class="modal-title">Edit Payment Voucher</h4>
                                  </div>
                                  {!! Form::open(array('route' =>['edit-payment-voucher', $data->tok],'method'=>'PUT')) !!}
                                  <div class="modal-body">
                                     <div class="row">
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                <label>Type</label><br>
                                                <input type="text" class="form-control" value="{{$data->otherpayment_type_object->name}}" readonly>
                                            </div>
                                            <div class="form-group">
                                                <label>Sub Type</label><br>
                                                <input type="text" class="form-control" value="{{$data->otherpayment_subtype_object->name}}" readonly>
                                            </div>
                                            <div class="form-group">
