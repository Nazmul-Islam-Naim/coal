...
<section class="content">
  ...
  <div class="row">
    {!! Form::open(array('route' =>['update-job-card'],'method'=>'POST')) !!}
    <div class="col-md-12">
      <div class="box box-primary">
        <div class="box-header with-border">
          <h3 class="box-title"> <i class="fa fa-exchange"></i> Add More Services</h3>
        </div>
        <!-- /.box-header -->
        <div class="box-body">
          <div class="row">
                  ...
                  <div class="row">
                      <div class="col-md-4">
                          <div class="form-group">
                            <?php
                                ...
                                $invNo = "";
                                if(!empty($count->job_card_no)){
                                    $invNo = $count->job_card_no + 1;
                                }else{
                                    $invNo = '10000';
                                }
                            ?>
                              <label>Job Card No</label>
                              <input type="text" name="job_card_no" value="{{$invNo}}" class="form-control" autocomplete="off">
                      ...
                      <div class="col-md-4">
                          <div class="form-group">
                              ...
                              <input type="text" name="receive_date_time" value="{{$singleData->receive_date_time}}" class="form-control datetimepicker" autocomplete="off">
                          </div>
                      </div>
                      <div class="col-md-4">
                          <div class="form-group">
                              <label>Delivery Date & Time</label>
                              <input type="text" name="delivery_date_time" value="{{$singleData->delivery_date_time}}" class="form-control datetimepicker" autocomplete="off">
                          </div>
                      </div>
                  </div>
                    ...
                    <tbody class="service_container"> 
                      ...
                      <tr id="div_{{$row_num}}">
                        <td style="width: 10%"> 
                          ...
                          <input type="hidden" name="service_details[{{$row_num}}][service_id]" value="{{$data->id}}">
                        </td>
                        <td> 
                          <input type="text" name="service_details[{{$row_num}}][instruction]" value="{{$data->instruction}}" class="form-control" id="instruction_{{$row_num}}" required="" autocomplete="off">
                        </td>
                        <td style="width: 20%"> 
                        ...
                        <td style="width: 10%">
                            <a href="{{URL::To('sells/delete-added-services/'.$data->id.'/'.$singleData->tok)}}" onclick="return confirm('Want To Delete? This will not be Undone...')" class="btn btn-sm btn-danger pull-right"><i class="fa fa-remove"></i></a>
                        </td> 
                      </tr> 
                  ...
                  <table class="table"> 
                    ...
                    <tbody class="parts_container"> 
                      ...
                      <tr id="div2_{{$row_num2}}"> 
                        ...
                        <td>
                            <a href="{{URL::To('sells/delete-added-parts/'.$data->product_id.'/'.$singleData->tok)}}" onclick="return confirm('Want To Delete? This will not be Undone...')" class="btn btn-sm btn-danger pull-right"><i class="fa fa-remove"></i></a>
                        </td>
                      </tr> 
        ...
        <div class="box-footer" style="padding: 0px;">
            <div class="text-right" style="position: sticky;bottom: 0;background: #337ab7;padding: 25px 75px 0px 75px;">
                <div class="row">
                  ...
                  <div class="col-md-2">
                    <div class="form-group" style="margin-top: -20px;">
                      <label for="Discount" style="color: #fff">Discount</label>
                      <input type="number" class="form-control" id="Discount" name="discount" onkeyup="discountFun(this.value);" value="{{$singleData->discount}}" autocomplete="off" {{($singleData->discount > 0) ? 'disabled':''}}>
                    </div>
                  </div>
                  <div class="col-md-2">
                    <div class="form-group" style="margin-top: -20px;">
                      <label for="Discount" style="color: #fff">Advance</label>
                      <input type="number" class="form-control" id="Advance" value="{{$singleData->advance}}" onkeyup="advanceFun(this.value);" autocomplete="off" readonly="">
                    </div>
                  </div>
                  <div class="col-md-2">
                    <div class="form-group" style="margin-top: -20px;">
                      <label style="color: #fff">Due</label>
                      <input type="number" id="dueAmount" class="form-control" value="{{$singleData->total_amount-($singleData->advance+$singleData->discount)}}" readonly="">
                    </div>
                  </div>
                  <div class="col-md-2">
                      <div class="form-group" style="margin-top: -20px;"> 
                        ...
                        <select class="form-control select2" name="payment_method" required=""> 
                          ...
                          <option value="{{$bank->id}}" {{($singleData->payment_method==$bank->id)?'selected':''}}>{{$bank->bank_name}}</option>
                          @endforeach
                        </select>
                      </div>
                  </div>
                  <div class="col-md-2">
                    <div class="form-group">
                      <input type="submit" name="purchase" class="btn btn-md btn-warning" value="Save Sell" style="padding: 10px 30px;font-weight: bold">
                    </div>
                  </div>
                </div>
            </div>
...
<script type="text/javascript">
  ...
  function addservice(){ 
    ...
    html += '<input type="text" class="form-control" value="J'+RowNum+'" readonly><input type="hidden" name="service_details['+RowNum+'][service_id]" value="0">';
    html += '</td>';
    html += '<td>'; 
  ...
  function calculate() {
    ...
    var TTlDiscount = document.getElementById('Discount').value;
    var TTlAmount = document.getElementById('HiddenTotalAmount').value;
    document.getElementById('dueAmount').value = parseFloat(TTlAmount)-parseFloat(TTlAdvance)-parseFloat(TTlDiscount);
  }
  ...
  function advanceFun(value) {
    ...
    var TTlDiscount = document.getElementById('Discount').value;
      var AmountAfterAdvance = parseFloat(GrandTotalHidden)-parseFloat(value)-parseFloat(TTlDiscount);
  ...
  function discountFun(value) {
    var GrandTotalHidden = document.getElementById('HiddenTotalAmount').value;
    var TTlAdvance = document.getElementById('Advance').value;
      var AmountAfterAdvance = parseFloat(GrandTotalHidden)-parseFloat(value)-parseFloat(TTlAdvance);
      if (isNaN(AmountAfterAdvance)) {
        document.getElementById('dueAmount').value = 0;
      }else{
        document.getElementById('dueAmount').value = AmountAfterAdvance.toFixed(2);
